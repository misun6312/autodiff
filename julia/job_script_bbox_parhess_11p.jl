...
function main()
    ...
    saveto_filename = *("parhess_julia_out_",ratname,"_rseed_11p_bbox.mat")
    ...
    params = [sigma_a, sigma_s_R, sigma_s_L, lam, B, bias, phi, tau_phi, lapse_R, lapse_L, input_gain_weight]   
    # params = [115.4371,   81.5764,    4.5508,   -3.6219,    8.5650,    1.1715,    0.0100,   0.0645,    0.0001,    0.1370, 0.3683]
    # params = [106.2984,   79.4005,    4.8972,   -3.4972,    8.5650,   1.0831,    0.2,    0.0679,    0.00001,    0.1344,    0.3615]
    # l = [0, 0, 0,       -5, 5,  -7, 0.0, 0.005, 0., 0., 0.]
    # u = [200, 200, 200, +5, 25, +7, 1.2,    1.5, 1., 1., 1.]
	...
	opars, traj, cost, cpm_traj, ftraj, H = bbox_Hessian_keyword_minimization(params, args_11p, bbox, func,
